...
function showOpmTip()
	closeOpmTip()
	GUIEditor.window[2] = guiCreateWindow(607, 304, 449, 280, "Mensaje privado sin conexión", false)
	...
	GUIEditor.label[1] = guiCreateLabel(17, 31, 414, 67, "El mensaje privado sin conexión es una función premium que te permite enviar un mensaje privado a cualquier jugador sin importar si está en línea o fuera de línea.\n\nEl jugador objetivo recibirá tu mensaje privado como una notificación que se muestra a continuación.:", false, GUIEditor.window[2])
	guiLabelSetHorizontalAlign(GUIEditor.label[1], "left", true)
	local perk = exports.donators:getPerks(37)
	GUIEditor.label[2] = guiCreateLabel(17, 197, 414, 30, "Cuesta "..perk[2].." GC(s) por mensaje.\nPara enviar mensajes privados sin conexión utilice /opm", false, GUIEditor.window[2])
...
function startOpm(cmd, username, ...)
	if not username or not (...) then
		...
		GUIEditor.window[1] = guiCreateWindow(664, 283, 438, 295, "Mensaje privado sin conexión", false)
		guiWindowSetSizable(GUIEditor.window[1], false)
		exports.global:centerWindow(GUIEditor.window[1])

		GUIEditor.label[1] = guiCreateLabel(20, 33, 50, 26, "Para:", false, GUIEditor.window[1])
		...
		GUIEditor.label[2] = guiCreateLabel(20, 59, 60, 26, "Mensaje:", false, GUIEditor.window[1])
		guiSetFont(GUIEditor.label[2], "default-bold-small")
		guiLabelSetVerticalAlign(GUIEditor.label[2], "center")
		GUIEditor.label[3] = guiCreateLabel(94, 63, 293, 16, "", false, GUIEditor.window[1])
		guiLabelSetHorizontalAlign(GUIEditor.label[3], "center", false)
		guiLabelSetVerticalAlign(GUIEditor.label[3], "center")
		GUIEditor.button[3] = guiCreateButton(22, 249, 197, 26, "Cancelar", false, GUIEditor.window[1])
		guiSetFont(GUIEditor.button[3], "default-bold-small")
		local cost = 0
		GUIEditor.button[2] = guiCreateButton(219, 249, 197, 26, "Enviar"..(cost > 0 and (" ("..cost.." GCs)") or ""), false, GUIEditor.window[1])
		...
		addEventHandler("onClientGUIChanged", GUIEditor.edit[1], function()
        	if source == GUIEditor.edit[1] then
        		...
        		if string.len(text) >= 3 then
	        		local count = 0
	        		guiSetText(GUIEditor.label[3], "Buscando..")
	        		...
	        		timer.username = setTimer(function()
	        			...
	        			if username then
	        				...
	        				guiSetText(GUIEditor.label[3], "Encontrado: "..username..".")
	        				...
	        				if count > 4 then
	        					guiSetText(GUIEditor.label[3], "Usuario no encontrado.")
