...
function showGeneralshopUI(shop_type, race, gender, discount, products1)
	...
	if not wCustomShop and not wAddingItemsToShop and not wGeneralshop and not getElementData(localPlayer, "shop:NoAccess" ) then
		...
		if shop_type==17 then
			...
			local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'"Bienvenido a '..bizName..'!" Haz doble clic en un artículo para comprarlo!',true,tProducts)
			...
			local colName = guiGridListAddColumn(gProducts,"Nombre",0.2)
			local colAmount = guiGridListAddColumn(gProducts,"Detalles",0.2)
			local colPrice = guiGridListAddColumn(gProducts,"Precio",0.1)
			local colDesc = guiGridListAddColumn(gProducts,"Descripcion",0.36)
			...
			if isClientBizOwner or exports.global:hasItem(localPlayer, 5, getElementDimension(localPlayer)) then
				----------------------START EDIT CONTACT-------------------------------------------------------
				tGoodBye = guiCreateTab ( "Editar información de contacto" , shopTabPanel )

				local lTitle1 = guiCreateLabel(11,19,716,56,("Editar información de contacto - "..bizName),false,tGoodBye)
				...
				local lOwner = guiCreateLabel(11,75,100,20,"- Dueño:",false,tGoodBye)
				local eOwner = guiCreateEdit(111,75,600,20,sOwner,false,tGoodBye)
				local lPhone = guiCreateLabel(11,95,100,20,"- Numero Telefono:",false,tGoodBye)
				...
				local lForums = guiCreateLabel(11,135,100,20,"((Nombre del foro)):",false,tGoodBye)
				...
				bSend = guiCreateButton(0.01, 0.88, 0.49, 0.1, "Guardar", true, tGoodBye)
				...
				local bClose = guiCreateButton(0.5, 0.88, 0.49, 0.1, "Cerrar", true, tGoodBye)
				...
				tBizManagement = guiCreateTab ( "Administración de Empresas" , shopTabPanel )

				GUIEditor_Label[1] = guiCreateLabel(11,19,716,56,"Administración de Empresas - "..bizName or "",false,tBizManagement)
				...
				guiSetText(lWelcomeText,'"¡Bienvenido jefe! ¿Cómo estás?" || '..currentCap..'/'..sCapacity..' products , Ingresos totales: $'..exports.global:formatMoney(sIncome)..'.')

				GUIEditor_Label[2] = guiCreateLabel(11,75,716,20,"- Capacidad: "..sCapacity.." (Número máximo de productos que la tienda puede contener, tienes que pagar $1/hora más por 5 productos adicionales)",false,tBizManagement)
				GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Ingresos: $"..exports.global:formatMoney(sIncome),false,tBizManagement)
				GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Pago del personal: $"..exports.global:formatMoney(sPendingWage).." ($"..exports.global:formatMoney(sCapacity/wageRate).."/hora)",false,tBizManagement)
				GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- Ganancia: $"..exports.global:formatMoney(sProfit),false,tBizManagement)
				GUIEditor_Label[6] = guiCreateLabel(11,155,57,19,"- Ventas: ",false,tBizManagement)
				...
				if sProfit < 0 then
					...
					if sProfit < (0 - warningDebtAmount) then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (ADVERTENCIA: Si tiene una deuda de más de $"..exports.global:formatMoney(limitDebtAmount)..", su personal dejará el trabajo)." )
						guiLabelSetColor ( GUIEditor_Label[5], 255, 0, 0)
				...
				elseif sProfit == 0 then
					...
					if sProfit < 500 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Promedio).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 255, 0)
					elseif sProfit >= 500 and sProfit < 1000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Bienn!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 245, 0)
					elseif sProfit >= 1000 and sProfit < 2000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Muy Bienn!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 235, 0)
					elseif sProfit >= 2000 and sProfit < 4000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Excelente!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 225, 0)
					elseif sProfit >= 4000 and sProfit < 8000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Locura!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
					elseif sProfit >= 8000 and sProfit < 20000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Impresionante!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
					elseif sProfit >= 20000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (El Padre de los Negocios!!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 195, 0)
					end
				end
				---------------------
				local bExpand = guiCreateButton(0.695, 0.48, 0.15, 0.1, "Expandir negocio", true, tBizManagement)
				...
				addEventHandler( "onClientGUIClick", bExpand, function ()
					...
					guiSetText(GUIEditor_Label[2], "- Capacidad: "..tostring(getElementData(ped, "sCapacity")).." (Número máximo de productos que la tienda puede contener, tienes que pagar $1/hora más por "..wageRate.." productos adicionales)")
					guiSetText(GUIEditor_Label[4] , "- Pago del personal: $"..exports.global:formatMoney(sPendingWage).." ($"..exports.global:formatMoney(getElementData(ped, "sCapacity")/wageRate).."/hora)")
				...
				-------------------------
				bShrink = guiCreateButton(0.845, 0.48, 0.15, 0.1, "Achicar Negocio", true, tBizManagement)
				...
				---------------------------
				local bClearSaleLogs = guiCreateButton(0.695, 0.58, 0.3, 0.1, "Borrar registros de venta", true, tBizManagement)
				...
				local bPayWage = guiCreateButton(0.695, 0.68, 0.3, 0.1, "Pagar al personal", true, tBizManagement)
				...
				--------------------------------
				local bCollectProfit = guiCreateButton(0.695, 0.78, 0.3, 0.1, "Recoger ganancias", true, tBizManagement)
				...
				local bClose = guiCreateButton(0.695, 0.88, 0.3, 0.1, "Cerrar", true, tBizManagement)
				...
				local lTitle1 = guiCreateLabel(11,19,716,56,(bizName.." - ¡Hasta luego!"),false,tGoodBye)
					--guiLabelSetVerticalAlign(lTitle1[1],"center")
					guiLabelSetHorizontalAlign(lTitle1,"center",false)
					guiSetFont(lTitle1, BizNoteFont)
				-- Fetching info
				...
				local lOwner = guiCreateLabel(11,75,716,20,"- Dueño: "..sOwner.."",false,tGoodBye)
				local lPhone = guiCreateLabel(11,95,716,20,"- Número de teléfono: "..sFormatedPhone.."",false,tGoodBye)
				local lEmail = guiCreateLabel(11,115,716,20,"- Correo Electronico: "..sEmail.."",false,tGoodBye)
				local lForums = guiCreateLabel(11,135,716,20,"- ((Nombre Foro: "..sForum.."))",false,tGoodBye)
				local lGuide = guiCreateLabel(0.01,0.5,1,0.1,"        'Oye, puedo pasar tu mensaje a mis jefes si quieres.': ",true,tGoodBye)

				local eBargainName = guiCreateEdit ( 0.01, 0.58, 0.19, 0.1,"tu identidad", true, tGoodBye)
				...
				if not cooldown or getTickCount() - cooldown > 60000*coolDownSend then
					...
					guiSetText(bSend, "(Puedes enviar otro mensaje en "..coolDownSend.." minuto(s).)")
				...
				addEventHandler( "onClientGUIClick", bSend, function()
					...
					if contactContent and contactContent ~= "" and contactContent ~= "content" then
						...
						triggerServerEvent("shop:notifyAllShopOwners", localPlayer , ped, "¡Hola jefe! Lea esto: '"..contactContent.."', dijo "..contactName..".")
						guiSetText(bSend, "(Puedes enviar otro mensaje en "..coolDownSend.." minuto(s).)")
						guiSetEnabled(bSend, false)
						guiSetText(eContent, "")
		...
		elseif shop_type==19 then --Faction Drop NPC - Weapons
			...
			wCustomShop = guiCreateWindow(X,Y,Width,Height,"NPC de caída de facción para armas",false)
			guiWindowSetSizable(wCustomShop,false)
			...
			local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'¡Haz doble clic en un artículo para comprarlo!',true,tProducts)
			bCloseStatic = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Cerrar", true, tProducts)
			guiSetFont(bCloseStatic, BizNoteFont2)
			addEventHandler( "onClientGUIClick", bCloseStatic,hideGeneralshopUI , false )
			...
			fdgw.colName = guiGridListAddColumn(fdgw.gProducts,"Nombre",0.18)
			fdgw.colPrice = guiGridListAddColumn(fdgw.gProducts,"Precio",0.08)
			fdgw.colDesc = guiGridListAddColumn(fdgw.gProducts,"Descripcion",0.4)
			fdgw.colQuantity = guiGridListAddColumn(fdgw.gProducts,"En stock",0.06)
			fdgw.colRestock = guiGridListAddColumn(fdgw.gProducts,"Rellenando en",0.15)
			...
			for _, record in ipairs(products) do
				...
				if pRestockIn and tonumber(pRestockIn) and pRestockInterval > 0 then
					...
					elseif pRestockIn == 1 then
						pRestockInFinal = "Mañana"
					elseif pRestockIn == 2 then
						pRestockInFinal = "Pasado Mañana"
					elseif pRestockIn > 2 then
			...
			local updateConfigGUI = function()
				if ped and tProducts and lWelcomeText then
					if getElementData(ped, "faction_belong") <= 0 then
						...
						guiSetText(lWelcomeText, "Este NPC requiere configuraciones de Lead Admin..")
					else
						guiSetEnabled(tProducts, true)
						if addItemBtn and isElement(addItemBtn) then
							guiSetEnabled(addItemBtn, true)
						end
						guiSetText(lWelcomeText, "Haga doble clic en un artículo para comprarlo!")
					end
				end
			end
			updateConfigGUI()
			...
			if canPlayerAdminShop(localPlayer) then
				addItemBtn = guiCreateButton(0.85-0.15, 0.90 , 0.15, 0.089, "Crear Item", true, tProducts)
				...
				tBizManagement = guiCreateTab ( "Configuraciones" , shopTabPanel )
				local l1 = guiCreateLabel(11,19,716,56,"Configuraciones de NPC de Drop de facción",false,tBizManagement)
				...
				local col = 200
				local xOffset = 30
				local lTeam = guiCreateLabel(xOffset,line*3,716,56,"Conceder accesos a la facción:",false,tBizManagement)
				guiSetFont(lTeam, "default-bold-small")
				local cFaction =  guiCreateComboBox ( xOffset+col,line*3,col*2,150, "Ninguno", false, tBizManagement )
				local counter = 0
				local comboIndex1 = {}
				...
				local lMember = guiCreateLabel(xOffset,line*4,716,56,"¿Quién puede comprar artículos de este NPC?:",false,tBizManagement)
				guiSetFont(lMember, "default-bold-small")
				local cMember =  guiCreateComboBox ( xOffset+col,line*4,col*2,56, "Nadie", false, tBizManagement )
				guiComboBoxAddItem(cMember, "Nadie")
				guiComboBoxAddItem(cMember, "Lideres")
				guiComboBoxAddItem(cMember, "Lideres Y Miembros")
				exports.global:guiComboBoxAdjustHeight(cMember, 3)
				guiComboBoxSetSelected ( cMember, getElementData(ped, "faction_access") )

				local bSaveNpcConfigs = guiCreateButton(0.85-0.15, 0.90 , 0.15, 0.089, "Guardar", true, tBizManagement)
				...
				local bCloseStatic2 = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Cerrar", true, tBizManagement)
			...
			if not shop or #shop == 0 then
				outputChatBox("Esto no es una tienda. Vayase.")
				hideGeneralshopUI()
				return
			end
			...
			if shop_type == 7 then
				if not exports.global:hasItem(localPlayer, 183) then -- Viozy Membership Card
					outputChatBox("Debe obtener una tarjeta de membresía de Viozy para comprar aquí!", 255, 0, 0)
					hideGeneralshopUI()
					return
				end
			end
			...
			for _, category in ipairs( shop ) do
				...
				local cName = guiGridListAddColumn(grid, "Nombre", 0.18)
				local cPrice = guiGridListAddColumn(grid, "Precio", 0.08)
				local cDescription = guiGridListAddColumn(grid, "Descripcion", govOwned and 0.7 or 0.6)
				local cQuantity = false
				if not govOwned then
					cQuantity = guiGridListAddColumn(grid, "Cantidad", 0.1)
				end
				...
				for _, item in ipairs( category ) do
					...
					if checkItemSupplies(shop_type, supplies, item.itemID, item.itemValue, getMetaItemName(item)) or govOwned then
						...
						if item.minimum_age and getElementData(localPlayer, "age") < item.minimum_age then
							guiGridListSetItemText( grid, row, cPrice, "◊ " .. item.minimum_age .. " o mayor", false, false )
						else
						...
						if not govOwned then
							if bandanas[item.itemID] then
								guiGridListSetItemText(grid, row, cQuantity, supplies["122"] or "Agotado", false, false)
							elseif item.itemID == 16 then
								guiGridListSetItemText(grid, row, cQuantity, supplies["16"] or "Agotado", false, false)
							elseif item.itemID == 114 then
								guiGridListSetItemText(grid, row, cQuantity, supplies["114:" .. vehicle_upgrades[tonumber(item.itemValue)-999][3]] or "Agotado", false, false)
							else
								local supply = supplies[tostring(item.itemID) .. ":" .. (item.itemValue or 1)] or supplies[tostring(item.itemID) .. ":" .. (item.itemValue or 1) .. ":" .. getMetaItemName(item)] or "Agotado"
				...
				local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'"Bienvenido a '..bizName..'!" ¡Haz doble clic en un artículo para comprarlo!',true,tab)
				guiLabelSetVerticalAlign(lWelcomeText,"center")
				guiLabelSetHorizontalAlign(lWelcomeText,"center",false)
				guiSetFont(lWelcomeText, BizNoteFont18)
				if isClientBizOwner then
					guiSetText(lWelcomeText,'"¡Bienvenido jefe! ¿Cómo estás?"')
				end
				bCloseStatic[_] = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Cerrar", true, tab)
			...
			if isClientBizOwner or exports.global:hasItem(localPlayer, 5, getElementDimension(localPlayer)) then
				----------------------START EDIT CONTACT-------------------------------------------------------
				tGoodBye = guiCreateTab ( "Editar información de contacto" , tabpanel )
				guiSetInputEnabled(true)
				showCursor(true)
				local lTitle1 = guiCreateLabel(11,19,716,56,("Editar información de contacto - "..bizName),false,tGoodBye)
					--guiLabelSetVerticalAlign(lTitle1[1],"center")
				...
				local lPhone = guiCreateLabel(11,95,100,20,"- Numero Telefono:",false,tGoodBye)
				local ePhone = guiCreateEdit(111,95,600,20,sPhone,false,tGoodBye)
				local lEmail = guiCreateLabel(11,115,100,20,"- Correo Electronico:",false,tGoodBye)
				local eEmail = guiCreateEdit(111,115,600,20,sEmail,false,tGoodBye)
				local lForums = guiCreateLabel(11,135,100,20,"((Nombre Foro)):",false,tGoodBye)
				local eForums = guiCreateEdit(111,135,600,20,sForum,false,tGoodBye)
				...
				local lBizNote = guiCreateLabel(0.01,0.5,1,0.1,"- Nota comercial:",true,tGoodBye)
				...
				addEventHandler( "onClientGUIClick", bSend, function()
					...
					if guiGetText(ePhone) ~= "" and not tonumber(guiGetText(ePhone)) then
						guiSetText(ePhone, "Numero Invalido")
					else
						triggerServerEvent("shop:saveContactInfo", localPlayer, ped, {guiGetText(eOwner),guiGetText(ePhone),guiGetText(eEmail),guiGetText(eForums)})
						hideGeneralshopUI()
					end
				...
				tBizManagement = guiCreateTab ( "Administación Negocio" , tabpanel )

				GUIEditor_Label[1] = guiCreateLabel(11,19,716,56,"Administación Negocio - "..bizName or "",false,tBizManagement)
					--guiLabelSetVerticalAlign(GUIEditor_Label[1],"center")
					guiLabelSetHorizontalAlign(GUIEditor_Label[1],"center",false)
					guiSetFont(GUIEditor_Label[1], BizNoteFont)
				...
				GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Ingreso: $"..exports.global:formatMoney(sIncome),false,tBizManagement)
				GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Pago del personal: $"..exports.global:formatMoney(sPendingWage),false,tBizManagement)
				GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- Ganancia: $"..exports.global:formatMoney(sProfit),false,tBizManagement)
				GUIEditor_Label[6] = guiCreateLabel(11,155,57,19,"- Ventas: ",false,tBizManagement)
				GUIEditor_Memo[1] = guiCreateMemo(11,179,498,184,sSales,false,tBizManagement)
				guiMemoSetReadOnly(GUIEditor_Memo[1], true)
				...
				elseif sProfit == 0 then
					...
					if sProfit < 500 then
						...
						guiLabelSetColor ( GUIEditor_Label[5], 0, 255, 0)
					elseif sProfit >= 500 and sProfit < 1000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Buena!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 245, 0)
					elseif sProfit >= 1000 and sProfit < 2000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Muy Buenaa!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 235, 0)
					elseif sProfit >= 2000 and sProfit < 4000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Excelente!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 225, 0)
					elseif sProfit >= 4000 and sProfit < 8000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Genial!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
					elseif sProfit >= 8000 and sProfit < 20000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Impresionante!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
					elseif sProfit >= 20000 then
						guiSetText(GUIEditor_Label[5] , "- Ganancia: $"..exports.global:formatMoney(sProfit).." (Dios de los negocios!!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 195, 0)
					end
				end
				...
				local bOrderSupplies = guiCreateButton(0.695, 0.48, 0.3, 0.1, "Gestionar Stock", true, tBizManagement)
				...
				local bClearSaleLogs = guiCreateButton(0.695, 0.68, 0.3, 0.1, "Borrar registros de venta", true, tBizManagement)
				guiSetFont(bClearSaleLogs, BizNoteFont2)
				addEventHandler( "onClientGUIClick", bClearSaleLogs, function ()
					guiSetText(GUIEditor_Memo[1],"")
					setElementData(ped, "sSales", "", true)
				...
				local bCollectProfit = guiCreateButton(0.695, 0.78, 0.3, 0.1, sIncome > sPendingWage and "Recoger ganancias" or "Resolver Salario", true, tBizManagement)
				guiSetFont(bCollectProfit, BizNoteFont2)
				if govOwned then
					guiSetEnabled(bCollectProfit, false)
				else
				...
				tGoodBye = guiCreateTab ( "Datos de contacto" , tabpanel )
				...
				addEventHandler( "onClientGUIClick", bSend, function()
					...
					if contactContent and contactContent ~= "" and contactContent ~= "content" then
						contactName = guiGetText(eBargainName):gsub("_"," ")
						if contactName == "" or contactName == "tu identidad" then
							contactName = "A Customer"
						else
							if getElementData(localPlayer, "gender") == 0 then
						...
						triggerServerEvent("shop:notifyAllShopOwners", localPlayer , ped, "¡Hola jefe! Lee esto '"..contactContent.."', dijo "..contactName..".")
						setElementData(localPlayer, "shop:coolDown:contact", getTickCount(), false)
						guiSetText(bSend, "(Puedes enviar otro mensaje en "..coolDownSend.." minuto(s).)")
						guiSetEnabled(bSend, false)
						guiSetText(eContent, "")
...
function showStockGUI(ped, shop, intelement, shop_type)
	...
	stockWindow = guiCreateWindow(511, 293, 485, 333, "Administrar Stocks", false)
    guiWindowSetSizable(stockWindow, false)

    local stockGrid = guiCreateGridList(10, 27, 439, 260, false, stockWindow)
    guiGridListAddColumn(stockGrid, "Producto", 0.5)
	guiGridListAddColumn(stockGrid, "$ Costo por Unidad", 0.2)
    guiGridListAddColumn(stockGrid, "Cantidad", 0.2)
				...
				if not bandanas[item.itemID] and item.itemID ~= 114 then
					if item.itemID == 16 then
						if not handledClothes then
							...
							guiGridListAddRow(stockGrid, "Toda la ropa", 40, tostring(clothes_quantity))
							handledClothes = true
						end
					else
						row = row + 1
	...
	local cost = 0
    local stockCost = guiCreateLabel(25, 302, 157, 15, "Costo Total: $0", false, stockWindow)
	guiSetFont(stockCost, "default-bold-small")
    local stockSave = guiCreateButton(336, 292, 114, 32, "Orden", false, stockWindow)
